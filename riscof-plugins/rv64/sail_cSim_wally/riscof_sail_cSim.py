    ...
    def runTests(self, testList, cgf_file=None, header_file= None):
        ...
        for file in testList:
            ...
            if ('NO_SAIL=True' in testentry['macros']):
                ...
                reference_output = re.sub("/src/","/references/", test)
                reference_output = reference_output[:-1] + "reference_output"
                execute += f'cut -c-{8:g} {reference_output} > {sig_file}' #use cut to remove comments when copying
            else:
                execute += self.sail_exe[self.xlen] + '  -i -v --trace=step --pmp-count=16 --pmp-grain=0 --ram-size=8796093022208 --signature-granularity=8  --test-signature={0} {1} > {2}.log 2>&1;'.format(sig_file, elf, test_name)
