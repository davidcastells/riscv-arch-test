    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            # simcmd = self.dut_exe + ' -m8796093022208 --isa={0} +signature={1} +signature-granularity=8 {2}'.format(self.isa, sig_file, elf)
            # simcmd = simcmd + ';' + self.dut_exe + ' -m8796093022208 --isa={0} --log-commits -l my.elf 2> {1}'.format(self.isa, log_file)
            simcmd = 'python  /home/dcr/INT_RISCV/punxa/test/riscv-arch-test/tb_signature.py  +signature={} +signature-granularity=8 {}'.format( sig_file, elf)
          else:
            simcmd = 'echo "NO RUN"'
